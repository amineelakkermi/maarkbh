...
export async function GET(request: NextRequest){
    try{
    const response = await fetch('http://139.59.140.232/api/customers/identity-types' , {
    method: 'GET',
    headers: { 
     ...
     return NextResponse.json({ error: 'Failed to fetch identity types' }, { status: response.status });
    }
